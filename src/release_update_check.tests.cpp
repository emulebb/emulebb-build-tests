...
namespace
{
	std::string BuildReleaseJson(const char *pszTagName, const char *pszAssetName, bool bDraft = false, bool bPrerelease = false)
	{
		return std::string("{\"tag_name\":\"") + pszTagName
			+ "\",\"html_url\":\"https://github.com/emulebb/emulebb/releases/tag/" + pszTagName
...
TEST_CASE("eMuleBB release tags use strict semver identity")
{
	SModReleaseVersion parsed = {};

	CHECK(TryParseReleaseTag("emulebb-v1.1.1", parsed));
	...
	CHECK_FALSE(TryParseReleaseTag("emulebb-v1.1", parsed));
	CHECK_FALSE(TryParseReleaseTag("emulebb-v1.1.1-beta", parsed));
	CHECK_FALSE(TryParseReleaseTag("emulebb-v42949672960.0.0", parsed));
}

TEST_CASE("eMuleBB release version comparison orders major minor and patch")
{
...
TEST_CASE("eMuleBB release evaluation requires current-platform ZIP asset")
{
	const SModReleaseVersion local = {1u, 1u, 1u};
	const std::string strX64Asset = BuildRequiredAssetName({1u, 1u, 2u}, "x64");

	const auto newer = EvaluateLatestReleaseJson(BuildReleaseJson("emulebb-v1.1.2", strX64Asset.c_str()), local, "x64");
	CHECK_EQ(newer.eStatus, EReleaseEvaluationStatus::Newer);
	CHECK_EQ(newer.strRequiredAssetName, "emulebb-1.1.2-x64.zip");

	const auto missingAsset = EvaluateLatestReleaseJson(BuildReleaseJson("emulebb-v1.1.2", "emulebb-1.1.2-arm64.zip"), local, "x64");
	CHECK_EQ(missingAsset.eStatus, EReleaseEvaluationStatus::MissingAsset);

	const auto notNewer = EvaluateLatestReleaseJson(BuildReleaseJson("emulebb-v1.1.1", "emulebb-1.1.1-x64.zip"), local, "x64");
	CHECK_EQ(notNewer.eStatus, EReleaseEvaluationStatus::NotNewer);
}

TEST_CASE("eMuleBB release evaluation ignores malformed and prerelease payloads")
{
	const SModReleaseVersion local = {1u, 1u, 1u};
	...
	const auto overflowedTag = EvaluateLatestReleaseJson(BuildReleaseJson("emulebb-v42949672960.0.0", "emulebb-42949672960.0.0-x64.zip"), local, "x64");
	CHECK_EQ(overflowedTag.eStatus, EReleaseEvaluationStatus::IgnoredRelease);

	const auto prerelease = EvaluateLatestReleaseJson(BuildReleaseJson("emulebb-v1.1.2", "emulebb-1.1.2-x64.zip", false, true), local, "x64");
