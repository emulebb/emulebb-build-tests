...
TEST_CASE("Upload queue clears retry cooldown only when queued peers request valid blocks")
{
	...
	CHECK(ShouldAttemptUploadRetryCooldownClearOnQueuedRequest(true, true, true));
	CHECK_FALSE(ShouldAttemptUploadRetryCooldownClearOnQueuedRequest(false, true, true));
	CHECK_FALSE(ShouldAttemptUploadRetryCooldownClearOnQueuedRequest(true, false, true));
...
{
	CHECK(ShouldAllowNoRequestCooldownClear(false, false, false));
	CHECK(ShouldAllowNoRequestCooldownClear(false, true, false));
	CHECK(ShouldAllowNoRequestCooldownClear(true, false, false));
	...
	CHECK_FALSE(ShouldClearActiveNoRequestCooldownOnQueuedRequest(true, false, true, 12, 12));
